# -*- coding: utf-8 -*-
"""
Created on Tue Feb 28 14:59:50 2017
...
#  filename=raw_input('Enter the filename: ')
  ...
  tokens=nlp.word_tokenize(str1);   #chracters are converted into tokens 
...
def initialize():
    ...
    token=nlp.word_tokenize(str_basic)
    countin=0
    for word in token:
    ...
    if i==0:
        for word in list_imp[2]:
            if word[0] in column_res(list_basic[index_class],0,5):
                ...
                tuple_b=(word[0],index_basic,1,len(word[1])/float(word_count),rc)
             ...
             for word in tuple_now[2]:
                    ...
                    if prev != 'NULL' and  prev not in column(list_all_words,0):
                          try:
                                ...
                                list_word.append((prev,prev_pos,dist,freq/float(word_count),rc))
                    ...
                    if nxt != 'NULL' and  nxt not in column(list_all_words,0):
                          try:
                                ...
                                rc=relation_coeff(dist,freq/float(word_count))
                                list_word.append((nxt,next_pos,dist,freq/float(word_count),rc))
...
listq=[]
finallist_all=[]
filepath1='E:/computerscience/my_projects/text_analytics/text_analytics/data/entertainment/e '
filepath2='E:/computerscience/my_projects/text_analytics/text_analytics/data/sports/s '
filepath3='E:/computerscience/my_projects/text_analytics/text_analytics/data/politics/p '
filepath4='E:/computerscience/my_projects/text_analytics/text_analytics/data/business/b '
filepath5='E:/computerscience/my_projects/text_analytics/text_analytics/data/technology/t '


#filepath1='E:/computerscience/my projects/text_analytics/text_analytics/entertainment_1/e '
#filepath2='E:/computerscience/my projects/text_analytics/text_analytics/sport_1/s '
#filepath3='E:/computerscience/my projects/text_analytics/text_analytics/politics_1/p '
#filepath4='E:/computerscience/my projects/text_analytics/text_analytics/business_1/b '
#filepath5='E:/computerscience/my projects/text_analytics/text_analytics/tech_1/t ' 
...
#filepath=[(filepath3,1),(filepath1,0),(filepath2,0),(filepath5,0),(filepath4,1)]
#filepath=[(filepath3,35),(filepath1,47),(filepath2,49),(filepath5,31),(filepath4,78)]
filepath=[(filepath3,10),(filepath1,503),(filepath2,10),(filepath5,575),(filepath4,610)]
     ...
     while(flag==0):
         ...
         if(len(basic_pred)>0):
            ...
            basic_word_used.append(list_basic)
             ...
             list_basic=[[],[],[],[],[]]
             temp=temp+1
             for l in range(temp*5,(temp+1)*5):
                 
                 if l>len(list_basic_ini[0])-1 and l>len(list_basic_ini[1])-1 and l>len(list_basic_ini[2])-1 and l>len(list_basic_ini[3])-1 and l>len(list_basic_ini[4])-1:
#                     for l in range(0,5):
#                       list_basic[0].append(token_politics[l])
#                       list_basic[1].append(token_entertain[l])
#                       list_basic[2].append(token_sport[l])
#                       list_basic[3].append(token_science[l])
#                       list_basic[4].append(token_business[l])
                     flag=1
                     break
                 else:
                     if l<len(list_basic_ini[0])-1:
                         list_basic[0].append(list_basic_ini[0][l])
                     if l<len(list_basic_ini[1])-1:    
                         list_basic[1].append(list_basic_ini[1][l])
                     if l<len(list_basic_ini[2])-1:
                         list_basic[2].append(list_basic_ini[2][l])
                     if l<len(list_basic_ini[3])-1:
                         list_basic[3].append(list_basic_ini[3][l])
                     if l<len(list_basic_ini[4])-1:
                         list_basic[4].append(list_basic_ini[4][l])
     ...
     if len(list_percentage)>0:
         ...
         if len(list_percentage)==1:
             if list_percentage[0][0]==(co) and list_percentage[0][1] > thresh:
                 success=success+1
    ...
    co=co+1 
    total_1.append((finallist_all,suc,success*100/float(len(suc)),((failure+(len(finallist_all)-len(suc)))*100/float(len(finallist_all))),(ambiguity*100/float(len(suc))),relation,avg/float(success),failure_class))
